...
namespace my_books_tests
{
    public class PublishersServiceTest
    {
        ...
        [Test, Order(5)]
        public void GetPublisherByIdWithResponse_Test()
        {
            for (int i = 1; i < 6; i++)
            {
                var result = publishersService.getPublisherById(i);
                Assert.That(result.Name, Is.EqualTo("Publisher "+i));
            }
        }

        [Test, Order(6)]
        public void GetPublisherByIdWithoutResponse_Test()
        {
            var result = publishersService.getPublisherById(1000);
